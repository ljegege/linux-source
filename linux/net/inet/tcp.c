 ...
 *	sk->inuse=1 or cli()
 */
// 从接收队列中查找是否有远端的数据包已经建立了连接，如果有则返回该数据包accept系统调用底层将调用这个函数
 ...
 */
// 关闭等待连接的套接字。所谓等待连接套接字就是对方发送SYN包后，应用程序尚未调用accept进行处理的数据包
 ...
 */
// 重传函数的总入口，主要完成首部的字段的重新设置和是否对所有的数据包进行重传。注意tcp会对其所有要发送的数据包缓存在重传队列中。
 ...
 */
// 复位重传定时器，就是将定时器从队列中删除，之后重新设置新的定时时间再重新插入定时器队列中
...
// 调用tcp_do_retransmit对数据进行重传，并更新从传信息和设置下一次重传的时间
