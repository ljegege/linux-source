 ...
 *  测试某个端口是否被使用，测试方法是遍历对应协议下的所有端口
 *  （端口号已经按照分成了SOCK_ARRAY_SIZE：256个队列，分类方式是将端口号对256取模）
 */
...
unsigned short get_new_socknum(struct proto *prot, unsigned short base)
{
	...
	// 如果有为空的队列则放回该队列的第一个端口号，否则返回拥有端口号最少的队列
